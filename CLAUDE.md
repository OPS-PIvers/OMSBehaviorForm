...
- `EmailTemplates.js`: `loadEmailTemplates()` builds the global `EMAIL_TEMPLATES` (`{openings, closings}`) from the **EmailTemplates** sheet (columns: Type, Category, Text, Transition, Active). Unlike `setupPillarsSheets()`, `setupEmailTemplatesSheet()` **never overwrites** an existing sheet — it only creates and seeds a missing one, so admin wording edits are permanent. Rows carry content-derived ids (`hashTemplateText`), so inserting or sorting rows doesn't invalidate a selection a teacher already made.
- Globals don't carry over between Apps Script executions. Every entry point (`doGet`, `processWebAppForm`, triggers, menu actions) must call `loadConstants()` / `loadPillarsData()` / `loadEmailTemplates()` before reading `CONFIG`, `PILLARS_DATA`, or `EMAIL_TEMPLATES`.
...
- Client → server calls go through `google.script.run`: `lookupStudent`, `getUserFullName`, `previewEmailBody`, and `processWebAppForm`. `processWebAppForm` sends the email, then calls `saveFormToSpreadsheetV2` to append the row.
- **Every submission passes through a required preview.** The submit button (`Preview Email`) only validates and calls `previewEmailBody`, which renders through the same `createSimplifiedEmailBody()` the send path uses — so the preview can't drift from the real email. `processWebAppForm` is only reached from the modal's Send Email button. Don't add a second send path that skips the modal.
- **Stop & Think requires an opening and a closing.** `#section-message-tone` is shown for `stopthink` only. Opening chips insert *opening + transition* into an editable textarea (`openingMessage`); the teacher's text is sent as-is, and it re-personalizes on a student-name change only while still unedited. Closing chips are a locked selection — the client sends `closingTemplateId` and the server resolves the wording via `resolveClosingMessage()`, so the admin's text is what goes out. Both are validated on the client and again in `processWebAppForm`.
- In `createSimplifiedEmailBody`, a Stop & Think opening **replaces** the old `"had a Stop and Think moment today"` sentence (a short `In <location> today, we observed:` lead-in takes its place), and the closing renders just above the existing `These moments provide opportunities…` paragraph. Both arguments are optional: `onFormSubmit` and the test functions pass nothing and fall back to the first active template of each type.
...
### Other entry points
- `EmailSystem.js`: `onOpen` builds the `Behavior System` menu (Admin > `Edit Email Opening & Closing Templates` opens the EmailTemplates sheet), and every admin/setup/test action is wired from it. It also has the email body builders (`createSimplifiedEmailBody` and the Good News / Stop & Think wrappers). `onFormSubmit` is the older Google Forms path, keyed on form question titles in `e.namedValues`. The web app is the current submission path.
